...
public class PrimitiveMapping implements BindMapping {
	...
	public PrimitiveMapping(JavaType javaType, Integer column, Collection<ValueHandler> valueHandlers) {
		Class<?> rawClass = javaType.getRawClass();
		if (String.class == rawClass) {
			cellMapping = new StringMapping(column, valueHandlers);
		} else if (Number.class.isAssignableFrom(rawClass)) {
			cellMapping = new NumberMapping(column);
		} else {
			cellMapping = new SimpleMapping(javaType, column);
		}
	}
