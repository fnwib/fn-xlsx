...
public class TitleResolver {
	...
	private PropertyConverter getPropertyConverter(Property property, Operation operation, final List<CellTitle> titles) {
		List<Integer> bindColumns = titles.stream().map(CellTitle::getCellNum).collect(Collectors.toList());
		...
		if (operation == Operation.LINE_NUM) {
			converter = new LineNumberConverter(property, bindColumns);
		} else if (javaType.isCollectionLikeType()) {
			converter = new CollectionConverter(property, bindColumns, valueHandlers);
		} else if (javaType.isMapLikeType()) {
			if (javaType.getKeyType().getRawClass() == Integer.class) {
				converter = new MapIntKeyConverter(property, bindColumns, valueHandlers);
			...
			if (titles.isEmpty()) {
				...
				converter = new BeanConverter(property, bindColumns.get(0), valueHandlers);
			} else {
